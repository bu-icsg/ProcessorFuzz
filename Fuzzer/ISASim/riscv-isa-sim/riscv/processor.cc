...
void processor_t::disasm(insn_t insn)
{
  ...
  if (last_pc != state.pc || last_bits != bits) {
    ...
    fprintf(log_file, "core %3d: 0x%016" PRIx64 " (0x%08" PRIx64 ") ",
            id, state.pc, bits );
    fprintf(log_file, "[0x%016" PRIx64 , state.mstatus);
    fprintf(log_file, ",%01x", state.frm);
    fprintf(log_file, ",%02x", state.fflags);
    fprintf(log_file, ",%02" PRIx64, state.mcause);
    fprintf(log_file, ",%02" PRIx64, state.scause);
    fprintf(log_file, ",%08" PRIx64, state.medeleg);
    fprintf(log_file, ",%08x", state.mcounteren);
    fprintf(log_file, ",%08x]", state.scounteren);
    //fprintf(log_file, " %08]" PRIx64, state.dcsr);
